...
- [ ] **Manage Identity card redesign (4-stage roll-out)** — parallel audit from designer + researcher + architect agents (2026-04-15) surfaced 13 interactive controls at primary tier (target 5), semantic duplicates, and real flow bugs. User decisions locked in: adopt Coinbase/Phantom "orange row until backup saved, then gone forever" pattern; skip the in-card AI button (researcher + architect both red-teamed it — every major product keeps AI outside the account menu; key-exfiltration and bad-advice-on-irreversible-actions risks outweigh benefit); rename header to "You". Stages:
  ...
  - **Stage 1b — Remaining bug fixes:** `runSaving` can't verify the actual download succeeded (popup blocker, disk full, CSP deny); add a mandatory "I saved it" confirmation gate before the new key commits. `fetchSourceTxsBatched` may 404 from `/api/tx-hex` on unconfirmed parent txs during sweep (WoC isn't always indexed); add BEEF or mempool-hex fallback. ~1–2h.
  - **Stage 2 — Dead-code cuts:** remove "Paste recovery key" textarea (redundant with file import), remove "Hide" button inside Show-recovery-key (dead micro-state), remove "Show recovery key" from main list (superseded by file backup, move behind "Advanced ▸" footer link), remove earnings sparkline from modal (visual noise; keep only in chip). ~1h.
  - **Stage 3 — Merge + reframe:** collapse "Secure identity" + "Change passphrase" into one "Passphrase" row (same code path, different entry state); move "Deposit" into balance zone as `+ Add funds` button; rename modal header "Manage Identity" → "You"; single persistent orange "Save your recovery file" banner above balance until `bsvibes_identity_backed_up=true`, then gone forever (Coinbase/Phantom one-time-nag pattern). ~2–3h.
  - **Stage 4 — Questions layout:** replace flat 9-item list with 3 intent-led questions ("Is my account backed up?", "I'm on a new device", "I think my keys were exposed") that expand in-place; pending-payment badge on balance ("$0.12 · 1 pending") with honest tooltip about sub-minute confirmation. ~3–4h.
