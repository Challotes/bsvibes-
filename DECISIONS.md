...
- **Wizard auto-close split (settled 2026-04-30, refined 2026-05-01).** `MoveAddressModal.onComplete` updates identity state only and sets `moveCompletedRef.current = true` (parent does not unmount the wizard); `onClose` (Continue button / X / backdrop on done, OR Cancel mid-wizard) is the single dismissal path that always closes the wizard but keeps the You modal open. On successful completion (`moveCompletedRef.current === true`), `onClose` additionally clears `manageAuthed` because the cached old re-auth passphrase is stale under the new passphrase — the You modal re-locks and the user re-enters the new passphrase to access rows. On Cancel mid-wizard, the manage gate stays unlocked. Rationale: prior `onComplete = close everything` pattern dismissed the wizard the instant `setStage("done")` fired, so the user never saw completed steps + sats-moved + safeguard copy. Prior Cancel = close everything pattern was inconsistent with RestoreModal Cancel (which returns to You modal). Ref instead of state to avoid stale-closure if React batches setStates before onClose runs.
...
- **Locked state: ambient pill + idea preservation (settled 2026-05-03).** Locked UI is a `🔒 Sign in` pill matching the identity-chip's bounding box (no Header reflow), NOT a 280px card replacement. Click pill → popover with input + "Enter" + "Need a hint?" link. Verbiage: **"Sign in"** (chip) and **"Enter"** (button) — neither "Login" (server implication wrong for local key decryption) nor "Unlock" (carceral, friction). Drops 💡 lightbulb in favor of click-to-reveal "Need a hint?" text link. PostForm textarea ENABLED when `needsUnlock && !identity` — user types their idea freely, the lock doesn't pre-announce in the input. Submit while locked: fire `signalLockedAttempt()` (shake + auto-expand pill for 8s), early-return WITHOUT calling `onPostCreated` (no phantom post). User enters passphrase, then retaps Enter (uncontrolled textarea preserves the typed content). **Reference apps:** 1Password locked vault (closest analogue), macOS screen saver / Touch ID, Notion offline mode, Slack offline typing. **Do not revert to the 280px card** — was rejected as visually dominating and forcing the user to enter passphrase before they could even capture their thought.

- **Universal "transaction action requires sign-in" pattern (settled 2026-05-03).** Every action needing a signed BSV identity (post, boot, reboot, tip, future features) starts with `if (!identity) { signalLockedAttempt(); return; }`. Buttons are never `disabled={!identity}` — site looks 100% normal locked, click fires shake, user retaps after sign-in. **No auto-replay.** The earlier `pendingSubmitRef` + auto-submit-useEffect pattern in PostForm was reverted because (a) Telegram / X / Slack / every mainstream chat all make the user redo the action after auth, (b) the auto-replay machinery added a class of bugs (stale closures, double-submit, phantom posts) for friction that doesn't exist when passphrase entry takes <10s, (c) the simpler pattern is universal — one line per handler vs a hook + promise + buffer abstraction. `LockedClickCatcher` stays mounted as the safety net for any future surface that forgets the explicit guard. **Considered and rejected:** `useGuardedAction(fn)` hook + `requestUnlock(): Promise<Identity>` — both architect and code-auditor agreed the abstraction collapses to dead code once auto-replay is dropped, since a promise nobody awaits is a function call.

- **Unlock-UI shake on locked-action — global pointerdown catcher (settled 2026-05-02).** Cold-load unlock UI (the `needsUnlock && !identity` branch in `IdentityChip`) shakes via `animate-[shake_0.5s_ease-in-out]` whenever the user attempts an identity-required action while locked. Same animation also fires on wrong-passphrase entry — single shake reused as the "this input needs attention" physical signal, error text differentiates the semantic. **Architecture:** the shake trigger is a single global `LockedClickCatcher` component (mounted inside `IdentityProvider` in `Feed.tsx`) that registers a `document.addEventListener("pointerdown", ..., {capture: true})` whenever the user is locked. On a pointerdown landing on an interactive element (`button, a[href], input, textarea, select, label, [role="button"], [role="link"], [tabindex]:not([tabindex="-1"])`) outside the unlock card (excluded via `data-unlock-ui="true"`) and not opted out (`data-bypass-lock-shake`), the catcher calls `signalLockedAttempt()` from the sibling `IdentityShakeContext`. Producers (`PostForm`, `BootButton`, `Bootboard` reboot) stay completely unaware of lock state — they keep `disabled={!identity}` honestly. Read-only browsing (scrolling, expanding sections, viewing posts/earnings) is unaffected because the selector targets only interactive elements. **Why `pointerdown`, not `click`:** per HTML5 spec, disabled form controls dispatch NO click events at all, so a click-capture handler would silently fail on the dominant locked-action case (clicks on disabled boot/post buttons). W3C Pointer Events DO fire on disabled elements — auditor caught this before commit. **Why a sibling context (`IdentityShakeContext`):** `useIdentityShakeKey` (counter) is consumed only by IdentityChip; `useIdentityShake` (stable callback) is consumed only by `LockedClickCatcher` and the IdentityChip wrong-passphrase handler. Splitting prevents the counter mutation from re-rendering any other context consumer. **Do not migrate back to per-site producer wiring** — that pattern was rejected as it leaks lock-state coupling into every identity-required feature, requires "looks-disabled-but-clickable" hacks that contradict `disabled={!identity}` honesty, and forces every new feature to remember the wiring forever. **Do not merge `shakeKey` into `IdentityContext`** — would re-render every `identity` consumer on every shake. **Per-site is the rare opt-out**, not the default — stamp `data-bypass-lock-shake` on a button if a future feature needs a different signal (e.g. "needs funding" → toast with deposit CTA).
