...
| `SignInModal`, `IdentityBar` (You modal), ~~`MoveAddressModal`~~ *(deleted 2026-06-14 with the key-rotation removal — other `MoveAddressModal` refs in this temp doc are likewise historical)*, `RestoreModal`, `ChangePassphraseModal`, `FundAddress` | **SHIPPED (Bucket 1):** all converted to the bottom-sheet pattern. Also: all 7 centered modals use `svh` instead of `vh` for top-offset + max-height so Android Chrome's toolbar can't clip them (2026-06-03). |
