...
const { bootPriceFloor, bootPriceCeiling, satsPerContributor, priceCacheTtlMs, activeWindowDays } = FAIRNESS_CONFIG;
...
export function getBootPriceForUser(
  ...
  const grant = db.prepare(
    'SELECT free_boots_used FROM boot_grants WHERE pubkey = ?'
