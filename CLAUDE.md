...
- `src/contexts/IdentityContext.tsx` — Shared identity provider (single BSV SDK load). Also exposes `useIdentityShake()` (stable `signalLockedAttempt()` callback, consumed by `LockedClickCatcher` and the wrong-passphrase handler in IdentityChip) and `useIdentityShakeKey()` (counter, consumed only by IdentityChip to drive the unlock-UI shake animation). Split into two sibling contexts so the few callers of the signal stay stable while the counter mutates.
