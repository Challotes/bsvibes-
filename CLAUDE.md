...
- `src/components/LockedClickCatcher.tsx` — Global pointerdown-capture handler. When `needsUnlock && !identity`, any pointerdown on an interactive element outside the unlock card calls `signalLockedAttempt()` so IdentityChip shakes. Producers stay unaware of lock state. Mounted inside `<IdentityProvider>` in `Feed.tsx`. Opt-out via `data-bypass-lock-shake` attribute. Uses `pointerdown` (not `click`) because disabled form controls suppress click events but still dispatch pointer events per W3C spec. **Safety net only** — explicitly-guarded handlers (see Universal contract below) are the recommended path; this catches anything that forgets.

### Universal contract: transaction action requires sign-in
...
```ts
if (!identity) { signalLockedAttempt(); return; }
```

Buttons stay visually identical locked vs unlocked (no `disabled={!identity}`, no greyed-out state — the site looks 100% normal). User taps → shake fires → sign-in popover opens → user enters passphrase → user retaps. No auto-replay buffering, no `pendingSubmitRef` machinery — Telegram/X/Slack convention. Adopted in PostForm `submitForm()`, PostList `BootButton.handleBoot()`, Bootboard `HistoryRow.handleReboot()`. Future toolkit features inherit the pattern with one import + one line.
...
- `src/contexts/IdentityContext.tsx` — Shared identity provider (single BSV SDK load). Also exposes `useIdentityShake()` (stable `signalLockedAttempt()` callback, consumed by `LockedClickCatcher`, the wrong-passphrase handler in IdentityChip, and every transaction-action handler — PostForm `submitForm`, PostList `BootButton.handleBoot`, Bootboard `HistoryRow.handleReboot`) and `useIdentityShakeKey()` (counter, consumed only by IdentityChip to drive the unlock-UI shake animation). Split into two sibling contexts so the many callers of the signal stay stable while the counter mutates.
