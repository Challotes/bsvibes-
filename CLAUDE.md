...
- `src/services/bsv/backup-template.ts` — Self-contained HTML recovery file generator + `downloadBackup(data)` (filename auto-built from `pathType + name + addr6 [+ to + newAddr6] + datetime`) / `getStoredHint` utilities. `BackupData.pathType` is required (`"save" | "rotation" | "pre-rotation" | "restore-pre"`); optional `oldAddress` triggers the `<oldAddr6>-to-<newAddr6>` segment in the filename. HTML template displays a public-address row above each WIF (Copy button on address only — no Copy on WIF), red per-WIF warning beneath, an extra "may still hold funds" line for the previous-key block, and a red `plaintext-banner` above the only path that produces a plaintext WIF. WIF text is `user-select: all` for OS-shortcut copy. **Do not pass filenames to `downloadBackup`** and **do not re-add Copy buttons on WIF surfaces** — see DECISIONS.md "Backup file audit & overhaul".
